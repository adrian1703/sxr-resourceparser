...
pages = [page.replace("ADDRESSBG-12", "SELLER TAX REPRESENTATIVE POSTAL ADDRESS BG-12") for page in pages]
#%% modify pages
lines = "\n".join(pages).split("\n")
...
df['Prefix'] = df['code'].str.extract(r'(^[A-Z]{2}-)', expand=False)
df['Numeric'] = df['code'].str.extract(r'(\d+$)', expand=False).astype(int)
